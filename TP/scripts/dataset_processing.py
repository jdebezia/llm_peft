...
def format_dolly(sample):
    ...
    Args:
        sample (dict): Dataset sample with instruction, context, and output
...
def prepare_datasets(
    ...
    Args:
        sagemaker_session_bucket (str, optional): S3 bucket for saving processed datasets
        train_size (int or float): Number or proportion of samples for training
        eval_size (int or float): Number or proportion of samples for evaluation
        test_size (int or float): Number or proportion of samples for testing
        train_seed (int): Seed for selecting the fixed train dataset
        eval_seed (int): Seed for selecting eval and test datasets
        chunk_length (int): Length of chunks to create during tokenization
    
    Returns:
        dict: Dictionary containing processed train, eval, and test datasets
    """
    # Process the raw dataset
    processed_dataset = process_sql_dataset()
    
    # Split the dataset
    dataset_splits = split_dataset_by_size(
        processed_dataset, 
    ...
    # Reset remainder for eval dataset
    remainder = {"input_ids": [], "attention_mask": [], "token_type_ids": []}
    
    # Prepare eval dataset
    eval_dataset = eval_dataset.map(template_dataset, remove_columns=list(eval_dataset.features))
    
    # Tokenize and chunk eval dataset
    lm_dataset_eval = eval_dataset.map(
    ...
    print(f"Total eval samples: {len(lm_dataset_eval)}")
    
    # Optionally save datasets to S3 (if SageMaker bucket is provided)
    if sagemaker_session_bucket:
        lm_dataset.save_to_disk(f's3://{sagemaker_session_bucket}/text_2_sql/dataset/train')
        lm_dataset_eval.save_to_disk(f's3://{sagemaker_session_bucket}/text_2_sql/dataset/valid')
        test_dataset.save_to_disk(f's3://{sagemaker_session_bucket}/text_2_sql/dataset/test')
    
    return {
        "train_tokenized": lm_dataset,
        "eval_tokenized": lm_dataset_eval,
        "original_splits": dataset_splits
    }
